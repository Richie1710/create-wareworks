...
public final class WareworksNetwork {
    ...
    private static void onTerminalRequest(TerminalRequestPayload payload, IPayloadContext context) {
        ...
        Optional<RequestResult> result = WarehouseTerminalMenu.submitRequest(player, payload.containerId(),
                payload.key(), payload.amount());
        if (result.isPresent() && player instanceof ServerPlayer serverPlayer)
            PacketDistributor.sendToPlayer(serverPlayer,
                    TerminalResultPayload.of(payload.containerId(), payload.key(), result.get()));
    }
