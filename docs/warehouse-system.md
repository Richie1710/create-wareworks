...
* **Storage priority (M16, issue #11, ADR-028).** A number **0..9** on the same value box decides which of the **equally suitable** storage locations the crane fills first, so a player can say "the vault by the door before the far end of the aisle".
  * **It is a property of the location**, like the filter (ADR-021) — not of the item, not of the controller. A **short click** on the filter slot still sets or clears the filter; **holding** the click opens Create's value settings board with one row, "Priority". A second value box does not fit on the plate (6 px tall, two boxes need 8 px between their centres) and every other face already carries an interaction, so one box carries both settings (§3.1.1, ADR-028).
  * **Default 0 = no preference**, higher fills first. A warehouse that sets no priority behaves **exactly** as before M16, which is proved rather than asserted: the planner's new sort key answers 0 for every pair, so the whole pre-M16 test suite is the regression proof (§7.4, ADR-028).
  * **It applies only when storing, and only below the rules above it.** The ranking is: the store filter → consolidation and item-type grouping → **the priority** → travel time (§7.1, §7.4). So a dedicated location still beats a prioritised unfiltered one, a prioritised location never collects every item type in the warehouse, and a rejecting filter is never overruled.
  * **Retrieval ignores it entirely.** Fetching always takes the shortest path, so a high priority can never send the crane past a nearer location that holds the same item. This is structural, not a check: the retrieval path passes a neutral priority literally (§7.4).
  * **Nothing is re-shuffled**, exactly as for a filter: raising a priority moves, drops or deletes nothing that is already stored, and a running job keeps its target — only the next trip goes to the new preferred rack.
  * **"Fill last" is not directly expressible** (accepted consequence, ADR-028): a player raises the other locations instead. The range is 0-based so it can be widened later without a save migration.
  * **Readable in the world:** the number is drawn on the plate itself while it is not 0 (`client.render.WarehouseInterfaceRenderer`) — once, from any distance up to Create's filter render distance, and not a second time when you aim at the block — and goggles read "Priority: N" on the interface and "Prioritised locations: N" on the controller.
* Goggles: address (or "not part of an aisle"), the store filter (or "Accepts everything"), the storage priority while it is set, attached block name, used/total slots, top 3 item types with counts, reserved amounts. Goggle data is kept fresh only while a player looks at the interface through goggles (§3.1.1).
...
* **Store filter (M8, ADR-021)**: `content.storage.StorageFilterBehaviour` (a Create `FilteringBehaviour`) with `content.storage.StorageFilterValueBox`; the controller side is `content.controller.AisleFilters` and `core.job.FilterMatch`.
  ...
  * **No count.** The slot has no `showCount()`: a storage location has no amount, so a filter *amount* is never set or synced. Until M16 that also meant `acceptsValueSettings()` was false and no hold-to-edit board opened at all; since M16 the board exists and carries the storage **priority** instead of an amount (below), so a short click sets or clears the filter and holding the click opens the board.
...
* **Storage priority (M16, issue #11, ADR-028)**: the same `content.storage.StorageFilterBehaviour` and the same `StorageFilterValueBox`; the controller side is `AisleFilters` again, and the planner side is `PlannerInput#storePriority` (§7.4).
  * **One box, two settings (the placement decision, measured).** The aisle face is a 3 px brass frame around a plate that is only **6 px** tall (x 3..13, y 3..9), with the arm port immediately above it at y 9..13 (`stacker-crane.md` §7.1). A value box is hit-tested within half its scale, so two boxes need their centres **8 px** apart — more than the plate is tall — and any second position either reaches into the arm's path, leaves the block, or overlaps the first box's sphere, where the behaviour that comes first in `SmartBlockEntity#getAllBehaviours` silently swallows the other. All other faces are taken (`FACING` → the attached inventory, lateral → the row-building placement rule, top/bottom → the wrench, and all four covered by neighbours in a rack wall), and putting a setting there is the ADR-022 mistake M10 had to undo. So the priority is a **board row on the existing box**: `acceptsValueSettings()`, `getValueSettings`, `setValueSettings`, `createBoard` (one row, `MIN_PRIORITY` 0 .. `MAX_PRIORITY` 9, milestones every `PRIORITY_MILESTONE_INTERVAL` = 3) and `formatValue`.
  * **`acceptsValueSettings()` is a constant `true`**, deliberately **not** Create's `isCountVisible()`, whose `getMaxStackSize() > 1` term would make the board unreachable as soon as a player used a non-stackable item as a plain filter. Consequence, recorded because it is a change in feel: a filter click now goes through Create's client warmup and fires on **release** instead of taking `ValueSettingsInputHandler`'s immediate server-side `onShortInteract` branch. `isCountVisible()` is overridden to `true` as well, so Create's `FilteringRenderer` adds `getAmountTip()` ("Hold to set the priority") as the third hover line. `getCountLabelForValueBox()` is overridden to be **always empty** (it cannot simply be dropped — Create's own implementation would put the *filter amount* there), so the box carries no number of its own: the number is drawn once, by the block renderer. Showing it in the corner of the box as well was the first attempt and was wrong — see the renderer bullet below.
  * **The M8 fake-player protection is intact**, and the Mechanical Arm passes through: `ValueSettingsInputHandler` asks `mayInteract` *before* its fake-player branch, and `mayInteract`/`bypassesInput` now also let a click with the **Mechanical Arm** item through to Create's arm target selection, mirroring `content.station.RequestFilterBehaviour` (M12, §3.2.2). Without that the slot drew a value box and a "click to set" hint while the arm item hovered it, and the click that should aim the arm did nothing at all.
...
* **No arm point for the warehouse interface, controller, stacker crane dock or rail.** The interface has no inventory
  ...
  * **M16 nevertheless gave the interface's own slot the same two overrides** (`StorageFilterBehaviour#bypassesInput` and
    `#mayInteract` for the Mechanical Arm item, §3.1.1). Being no arm target is what makes them necessary: a click with
    the arm item on an interface should **place** the arm, and until M16 that click hit the value box on the aisle face
    and did nothing at all, with a "click to set" hint hovering a slot the arm can never use — the same leftover M12
    fixed on the output, on the one block whose aisle face a player in the aisle clicks most.
...
* **`JobPlanner<K, L>`:** `plan(PlannerInput)` → `PlanResult` (optional `PlannedJob` = job + estimated ticks, the `NoJobReason`s, the next input cursor); `planReroute(input, key, amount, type, failedTarget)` → optional `RerouteTarget`. The planner is pure: the controller supplies index, ledger view, locations, an `InsertEstimate` (default `fromSnapshots`) and simulated item handler calls (`LiveExtract`, `LiveInsert`), then reserves and assigns the returned job itself (§7.1 step 3). §7.1 is implemented as written (retrieve first for the oldest request, candidates and ranking, amount formulas, live validation with fall-through, store consolidation before travel time, round robin over inputs), with these **refinements**:
  ...
  * **Storage priorities (M16, issue #11, ADR-028):** `PlannerInput#storePriority` is a `ToIntFunction<? super L>` — a function of the **location** and of nothing else, which is the type-level guarantee that a priority can never depend on the item — and its builder default is `PlannerInput.NO_PRIORITY` (0 everywhere). It became the **fourth** key of the candidate ranking, `Integer.compare(b.priority(), a.priority())` (higher first, compared by swapping the operands rather than negating, so a range widened later cannot trip over `-Integer.MIN_VALUE`), between item-type compatibility and travel time. The ranking therefore reads **hard rules** (the store filter) → **automatic tidiness** (consolidation, grouping) → **explicit player preference** (the priority) → **cost** (travel time) → **stability** (index order), which is the shape a later port or collection preference (issue #12) can extend without touching the other keys.
